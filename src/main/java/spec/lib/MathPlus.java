...
/**
 * This class contains physical constants and special functions not found in the java.lang.Math
 * class. Like the java.lang.Math class this class is final and cannot be subclassed. All physical
 * constants are in cgs units.
 *
 * <p><B>NOTE:</B> These special functions do not necessarily use the fastest or most accurate
 * algorithms.
 *
 * @version $Revision: 1.1.1.1 $, $Date: 2004/06/11 10:15:29 $
...
public final class MathPlus extends Object {
   ...
   *     <p><FONT size=2> Converted to Java from<br>
  ...
  */
  ...
  /**
   * Returns the area under the left hand tail (from 0 to x) of the Chi square probability density
  ...
  /**
   * Returns the area under the right hand tail (from x to infinity) of the Chi square probability
  ...
  /**
   * @param a double value
   * @return The area under the Gaussian probability density function, integrated from minus
   *     infinity to x:
   */
  ...
  */
  ...
  /** Takes a one-dimensional array of integers and returns the maximum value in the array */
  public static final int getMaxValue(int[] anArray) throws ArithmeticException {

    int numIteration = (anArray.length / 4) * 4;

    int maxValue = Integer.MIN_VALUE;
    int tmp1, tmp2, tmp3;

    for (int i = 0; i < numIteration; i = i + 4) {
      tmp1 = Math.max(anArray[i], anArray[i + 1]);
      tmp2 = Math.max(anArray[i + 2], anArray[i + 3]);
      tmp3 = Math.max(tmp1, tmp2);

      maxValue = Math.max(maxValue, tmp3);
    }

    for (int i = numIteration; i < anArray.length; i++) {
  ...
  /** Takes a one-dimensional array of doubles and returns the maximum value in the array */
  public static final double getMaxValue(double[] anArray) throws ArithmeticException {
  ...
  /** Takes a one-dimensional array of integers and returns the minimum value in the array */
  public static final int getMinValue(int[] anArray) throws ArithmeticException {
    ...
    for (int i = 0; i < numIteration; i = i + 4) {
      tmp1 = Math.min(anArray[i], anArray[i + 1]);
      tmp2 = Math.min(anArray[i + 2], anArray[i + 3]);
      tmp3 = Math.min(tmp1, tmp2);

      minValue = Math.min(minValue, tmp3);
    }

    for (int i = numIteration; i < anArray.length; i++) {
      minValue = Math.min(minValue, anArray[i]);
    }

    return (minValue);
  }

  /** Takes a one-dimensional array of doubles and returns the minimum value in the array */
  public static final double getMinValue(double[] anArray) throws ArithmeticException {
  ...
  /** Takes a two-dimensional array of integers and returns the maximum value in the array */
  public static final int getMaxValue(int[][] anArray) throws ArithmeticException {
    int maxValue = Integer.MIN_VALUE;

    for (int i = 0; i < anArray.length; i++) {
      maxValue = Math.max(maxValue, getMaxValue(anArray[i]));
    }

    return (maxValue);
  }

  /** Takes a two-dimensional array of doubles and returns the maximum value in the array */
  public static final double getMaxValue(double[][] anArray) throws ArithmeticException {
    double maxValue = -1.0 * Double.MAX_VALUE;
  ...
  /** Takes a two-dimensional array of integers and returns the minimum value in the array */
  public static final int getMinValue(int[][] anArray) throws ArithmeticException {
    int minValue = Integer.MAX_VALUE;

    for (int i = 0; i < anArray.length; i++) {
      minValue = Math.min(minValue, getMinValue(anArray[i]));
    }

    return (minValue);
  }

  /** Takes a two-dimensional array of doubles and returns the minimum value in the array */
  public static final double getMinValue(double[][] anArray) throws ArithmeticException {
    double minValue = Double.MAX_VALUE;
  ...
  // NOTE: You cannot byteSwap a byte ... makes no sense
  /**
   * Given an integral number, byte swap it ... Thus, take it from a big to a little endian and vice
   * versa
   */
  public static short byteSwap(short s) {
    ...
    /*
    ***************************************
    short s2 = (short) ( ( ((short)b1 & 0xff) << 8) | ((short)b2 & 0xff));
    ***************************************
    */

    short s2 = (short) (((b1 & 0xff) << 8) | (b2 & 0xff));
  ...
  public static int byteSwap(int i) {
    ...
    int i2 = ((b1 & 0xff) << 24) | ((b2 & 0xff) << 16) | ((b3 & 0xff) << 8) | (b4 & 0xff);

    return i2;
  }

  public static long byteSwap(long l) {
